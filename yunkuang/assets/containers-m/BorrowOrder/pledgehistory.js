...
export default () => {
    ...
    const [margincurry, setMargincurry] = useState()
    const [sidenum, setSidenum] = useState()
    const { marginCurrencies } = useSelector((state) => ({
                ...
                <div className="css-y9kj8x">
                    ...
                    <div className="sc-htpNat cyiKwg">
                        ...
                        <button data-bn-type="button" className="css-dq59xs" onClick={() => {
                            if (endtime || margincurry || orderCodeid.current.value !== '' || sidenum || starttime) {
                                dispatch.loan.pledgeChange({
                                    endTime: Date.parse(endtime),
                                    marginCurrency: margincurry,
                                ...
                                <table style={{ tableLayout: 'fixed' }}>
                                    ...
                                    <thead>
                                        <tr>
                                            ...
                                            <th className="rc-table-cell">调整时间</th>
                                            <th className="rc-table-cell">抵押币种</th>
                                            <th className="rc-table-cell">调整方向</th>
                                            <th className="rc-table-cell">调整金额</th>
                                            <th className="rc-table-cell">调整前质押率</th>
                                            <th className="rc-table-cell">调整后质押率</th>
                                        ...
                                        tzlist.map((item, index) => {
                                            return <thead style={{ background: '#fff', borderBottom: '1px solid #eee' }} key={index}>
                                                <tr>
                                                    ...
                                                    <th className="rc-table-cell">{formatTime(item.addedAt, 'yyyy-MM-dd hh:mm:ss')}</th>
                                                    <th className="rc-table-cell">{item.marginCurrency}</th>
                                                    <th className="rc-table-cell">{item.side === 0 ? '转入调低' : '转出调高'}</th>
                                                    <th className="rc-table-cell">{item.pledgeDelta}</th>
                                                    <th className="rc-table-cell">{formatPrice(item.beforeRate * 100)}%</th>
                                                    <th className="rc-table-cell">{formatPrice(item.afterRate * 100)}%</th>
                    ...
                    {tzlist.length > 0 ? (
                        tzlist.map((item, index) => {
                            return <div className="css-jg500d" key={index}>
                                ...
                                <div className="css-ymjgx1">
                                    <div data-bn-type="text" className="css-1qp5tsr">调整时间</div>
                                    <div data-bn-type="text" className="css-1r2jm08">{formatTime(item.addedAt, 'yyyy-MM-dd hh:mm:ss')}</div>
                                </div>
                                <div className="css-ymjgx1">
                                ...
                                <div className="css-moreymjgx1">
                                    <div className="css-ymjgx1">
                                        <div data-bn-type="text" className="css-1qp5tsr">调整方向</div>
                                        <div data-bn-type="text" className="css-1r2jm08">{item.side === 0 ? '转入调低' : '转出调高'}</div>
                                    </div>
                                    <div className="css-ymjgx1">
                                        <div data-bn-type="text" className="css-1qp5tsr">调整金额</div>
                                        <div data-bn-type="text" className="css-1r2jm08">{item.pledgeDelta}</div>
                                    </div>
                                    <div className="css-ymjgx1">
                                        <div data-bn-type="text" className="css-1qp5tsr">调整前质押率</div>
                                        <div data-bn-type="text" className="css-1r2jm08">{formatPrice(Number(item.beforeRate) * 100)}%</div>
                                    </div>
                                    <div className="css-ymjgx1">
                                        <div data-bn-type="text" className="css-1qp5tsr">调整后质押率</div>
                                        <div data-bn-type="text" className="css-1r2jm08">{formatPrice(Number(item.afterRate) * 100)}%</div>
                                    </div>
                                    <div className="css-ymjgx1">
