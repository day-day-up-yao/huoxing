...
import { numToFixed } from '../../public/js/index'
...
export default () => {
    ...
    useEffect(() => {
        ...
        dispatch.public.quoteRates({
            ...
            console.log(res)
            if (res.code === 0) {
                setAllmoney(res.data[0].rates.USD)
            }
        }).catch(function (err) {
            console.log(err)
                    ...
                    if (Array.isArray(res)) {
                        ...
                        if (res.length === 0) {
                            ...
                            res.map((item) => {
                                ...
                                // if (item.tokenId.toUpperCase() === 'HT') {
                                //     datas.push(item)
                                //     hasHt = true
                                // }
    ...
    const tosetpassd = useCallback(() => {
        window.location.href = '/secure/enmoneypasswd'
    })
    ...
    const tobindemail = useCallback(() => {
        window.location.href = '/secure/enbindemail'
    })
    // 绑定手机
    const tobindmobile = useCallback(() => {
        window.location.href = '/secure/enbindmobile'
    })
    // 获取手机或邮箱验证码
    ...
    const handlesuccess = useCallback(() => {
        dispatch.public.withDrawal({
            ...
            chain_type: tokenid === 'USDT' ? selectusdname : tokenid === 'BTC' ? 'BTC' : '',
            auto_convert: true,
    ...
    const handlelastSucc = useCallback(() => {
        ...
        dispatch.public.withDrawalverify({
            ...
            if (!res.code) {
                window.location.href = `/secure/enproperty/${tokenid}?detail=1`
            } else {
                Toast.info(res.msg)
            }
        })
    ...
    const ratelist = [
        ...
        // { name: 'HT', flage: htlist },
        { name: 'FIL', flage: fillist }
    ]
    return <div className="enproperty">
        <div className="property-con">
            <div className="property-c-top">
                <h3>{t('property.property')}</h3>
                <div className="property-c-top-right">
                    <div className="property-c-top-r">
                        <h4>{t('property.allmoney')}</h4>
                        <p>
                            <span>{getFullNum(allnumber).toFixed(8)} BTC </span>
                            <span> (${numToFixed(allmoney * getFullNum(allnumber), 0)})</span>
                        </p>
                    </div>
                        ...
                        <li>
                            <span onClick={() => {
                                ...
                                dispatch.order.getAddress({
                                    token_id: item.name,
                                    chain_type: item.name === 'USDT' ? 'TRC20' : item.name === 'BTC' ? 'BTC' : ''
                                }).then((res) => {
                            ...
                            <span onClick={() => {
                                window.location.href = `/secure/enproperty/${item.name}`
                            }}>{t('property.mx')}</span>
                        </li>
                    </ol>
                })}
                        ...
                        <li>
                            <span onClick={() => {
                                ...
                                dispatch.order.getAddress({
                                    ...
                                    chain_type: item.tokenId === 'USDT' ? 'TRC20' : item.name === 'BTC' ? 'BTC' : ''
                                }).then((res) => {
                                    if (res.allowDeposit === true) {
                                        setAccording(true)
                                        setFlag(true)
            ...
            <div className="btnalert" style={{ display: flag === true ? 'flex' : 'none' }}>
                ...
                <div className="order-topup" style={{ display: according === true ? 'flex' : 'none' }}>
                    <div className="order-top-up">
                        <div className="order-top-posit" onClick={() => {
                            setAccording(false)
                            setFlag(false)
                        }}>close</div>
