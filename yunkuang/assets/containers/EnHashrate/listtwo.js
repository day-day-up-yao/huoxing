...
export default ({ i18n, t, minlist, setMinlist, idstr, incomeid, electype }) => {
    ...
    const handleToselect = useCallback((itm, idx) => {
        ...
        dispatch.order.inconeList({
            ...
            if (res.code === 0) {
                var datas = []
                const filinfo = res.data.filter((item) => {
                    return item.currency === 'FIL' || item.currency === 'XCH' || item.currency === 'CRU'
                })
                // console.log(filinfo)
                filinfo.map((value) => {
                    ...
                    minlist.map((item, index) => {
                        return <ul key={index}>
                            ...
                            <li>
                                ...
                                {(item.orderState === 0 || item.orderState === 4 || item.orderState === 6) && <span onClick={() => { handlepaytype(item) }}>{t('hasgrate.qhkf')}</span>}
                            </li>
                        </ul>
                    })
                ) : ('')}
