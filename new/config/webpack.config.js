...
const MULTI_PATH = path.resolve(ROOT_PATH, '_multiappsharing')
...
const getConfig = (target, server) => {
    ...
    const rules = [
        {
            test: /\.(js|jsx)?$/,
            include: [ASSETS_PATH, STATIC_PATH, CONFIG_PATH, BROWSER_PATH, SERVER_PATH, MULTI_PATH],
            use: [
                {
        ...
        {
            test: /\.(png|jpg|jpeg|gif|svg|svgz|ico|eot|ttf|woff)?$/,
            include: [ASSETS_PATH, STATIC_PATH, MULTI_PATH], 
            // include: [ROOT_PATH],
            use: [
                {
                    loader: 'url-loader',
