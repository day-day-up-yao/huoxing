...
const GameOverviewWeeklyNumberOfGamesChart = (props: any) => {
  const theme = useTheme();
  const option = useMemo(
    () => ({
      title: {
        text: 'Weekly Number of Games',
        textStyle: {
        ...
        formatter: (params: any) => {
          ...
          for (let i = 0; i < params.length; i++) {
            ...
            if (sum >= (window as any).mouseCurValue) {
              dom = `
                          <div class="game-overview-weekly-number-of-games-chart-tooltip">
                              ...
                              <div class="tooltip-item">
                                  <div class="tooltip-item-left">
                                      Distinct values of protocol slug:
                                  </div>
                                  <div class="tooltip-item-right">
                                      ${series.value}
                                  </div>
      ...
      yAxis: [
        {
          name: 'Distinct values of protocol slug',
          nameLocation: 'middle',
          nameGap: 60,
          type: 'value',
          silent: false,
  ...
  useEffect(() => {
    // 获取图表容器
    const ele = document.querySelector('#weekly-number-of-games-chart');
  ...
  useEffect(() => {
    ...
    dataList.map((item) => {
      series.map((item1: any) => {
        if (item1.name === item[1] && timeData.indexOf(item[0]) > -1) {
          item1.data[timeData.indexOf(item[0])] = item[2];
        }
      });
    });
  ...
  return (
    <div className="game-overview-weekly-number-of-games-chart" id="weekly-number-of-games-chart" />
  );
};

export default GameOverviewWeeklyNumberOfGamesChart;
