...
import { Enter, InputProps, TextAreaProps } from './helper';

export type NormalizeHandlerType = 'onBlur' | 'onPressEnter';
...
export default function useComposition({
  ...
  normalizeHandler?: (type: NormalizeHandlerType) => InputProps['normalize'];
}): {
  ...
  return {
    ...
    valueChangeHandler: (e: any) => {
      ...
      if (!refIsComposition.current) {
        ...
        if (compositionValue) setCompositionValue('');
    ...
    keyDownHandler: (e: any) => {
      ...
      if (!refIsComposition.current) {
        ...
        if (keyCode === Enter.code) {
          ...
          if (normalize) triggerValueChangeCallback(normalize(e.target.value), e);
        }
      }
    },
  };
