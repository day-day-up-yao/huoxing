...
export const getLocalInfo = (sendData, fn) => {
    return (dispatch) => {
        ...
        if (!article) {
            ...
            dispatch(addPostData({'info': JSON.parse(article)}))
            if (fn) {
...
// 获取twitter聚合的数据
export const getTwitterInfo = (sendData, fn) => {
    return (dispatch) => {
        axiosAjax('post', '/crawler/social/source/importNewsInfo', {...sendData}, function (res) {
            fn && fn(res.obj)
            if (res.code === 1) {
                console.log(res.obj)
                const actionData = res.obj
...
// 获取twitter聚合的策略
export const getTacticsInfo = (sendData, fn) => {
    return (dispatch) => {
        axiosAjax('post', '/crawler/social/source/importStrategyInfo', {...sendData}, function (res) {
            fn && fn(res.obj)
            if (res.code === 1) {
                const actionData = res.obj
                dispatch(addPostData({'info': actionData}))
