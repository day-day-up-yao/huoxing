...
import {axiosAjax, cutString, bannerStatusOptions} from '../../public/index'
...
class BannerIndex extends Component {
    ...
    componentWillMount () {
        ...
        columns = [{
            ...
            render: (record) => {
                if (record.type === 1) {
                    return <a target="_blank" href={record.typeLink && record.typeLink.trim() !== '' ? 'http://www.huoxing24.com/newsdetail/' + record.typeLink + '.html' : '#/banner-topList'}>{`${record.typeLink && record.typeLink.trim() !== '' ? 'http://www.huoxing24.com/newsdetail/' + record.typeLink + '.html' : '暂无链接'}`}</a>
                } else if (record.type === 2) {
                    ...
                    return <a target="_blank" href={record.typeLink && record.typeLink.trim() !== '' ? record.typeLink : '#/banner-topList'}>{record.typeLink && record.typeLink.trim() !== '' ? record.typeLink : '暂无链接'}</a>
                } else {
                    return <p>暂无</p>
                }
            }
    ...
    searchBanner = () => {
        ...
        dispatch(setFilterData({'status': this.state.status}))
        this.doSearch('init', {status: this.state.status, search: this.state.searchVal})
    }
    searchTitleVal = (event) => {
        this.setState({
                ...
                <Col>
                    ...
                    */}
                    <Input
                        ...
                        placeholder="标题"
                        defaultValue={''}
                        onChange={this.searchTitleVal}
                        onPressEnter={() => {
                            this.searchBanner()
                        }}
                    />
                    <Button type="primary" onClick={this.searchBanner}>搜索</Button>
