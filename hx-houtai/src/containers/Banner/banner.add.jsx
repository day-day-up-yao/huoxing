...
class BannerSend extends Component {
    ...
    componentDidUpdate (prevProps, prevState) {
        if (prevProps.bannerInfo.typeLink !== this.props.bannerInfo.typeLink) {
            this.setState({specialActive: this.props.bannerInfo.typeLink})
        }
    }
    componentWillMount () {
            ...
            if (!err) {
                ...
                switch (this.state.type) {
                    ...
                    // case '4':
                        ...
                        // delete this.state.specialActive
                        // break
                    case '5':
                ...
                console.log(this.state.type === '4')
                if (this.state.type === '4') {
                    values.typeLink = this.state.specialActive
                } else {
                    values.typeLink = typeLink
                }
                console.log(values.typeLink)
    ...
    specialKeyDown = (event) => {
        let code = event.keyCode
        if (code === 46 || code === 8) {
            this.setState({ModifyActive: ''})
        }
    }

    specialValClick = (event) => {
        let val = event.target.getAttribute('data-val')
        console.log(`${val}/${this.state.specialActive}`)
    ...
    render () {
        ...
        console.log(this.state.ModifyActive)
        const uploadButton = (
            <div>
                <Icon type="plus"/>
        ...
        let formItem = () => {
            if (this.state.type === '2') {
                ...
                return <FormItem
                    ...
                    label={label()}
                >
                    <div className="special-cont" style={{ marginLeft: '15px' }}>
                        <Input placeholder='示例：MarsBit特训营/20180725170844035121'
                            value={this.state.specialActive}
                            onChange={this.specialIdChange}
                            onKeyDown={this.specialKeyDown}/>
