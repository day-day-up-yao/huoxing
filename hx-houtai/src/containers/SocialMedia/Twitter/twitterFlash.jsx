...
class TwitterIndex extends Component {
        ...
        dispatch(setFilterData(
            {
                ...
                filterCat: location.query.filterCat || '0'
            }))
        this.doSearch('init')
        columns = [{
            title: <h3>ID</h3>,
            ...
            render: (item) => {
                ...
                return <div className="btn">
                    ...
                    */}
                    ...
                    <p>
                        <a
                            onClick={() => item.canImport === 1 ? this.insertItem({item: item}) : ''}
                            className="opt-btn"
                            href="javascript:void(0)"
                            style={{background: item.canImport === 1 ? '#ff9b6e' : '#afafaf'}}
                        >导入</a>
                    </p>
                </div>
            }
    ...
    insertItem (data) {
        ...
        if (filter.type === '2') {
            ...
            // let tr = url.match(/(\S*)#wechat_redirect/)[1]
            // console.log(tr, url)
            window.open(`http://${window.location.host}/#/post-send?from=twitter&urls=` + encodeURIComponent(data.item.url) + `&newsid=${data.item.id}`)
        } else if (filter.type === '5') {
            window.open(`http://${window.location.host}/#/post-send?from=tactics&urls=` + encodeURIComponent(data.item.url) + `&newsid=${data.item.id}`)
        } else {
    ...
    changePage (page) {
        ...
        hashHistory.push(`/socialMedia-twitter?page=${page}&filterCat=${filter.filterCat}&type=${filter.type}`)
    ...
    handleChange = (value) => {
        ...
        hashHistory.push(`/socialMedia-twitter?page=1&filterCat=${filter.filterCat}&type=${value}`)
    ...
    handleImpChange = (value) => {
        const {dispatch, filter} = this.props
        dispatch(setFilterData({'filterCat': value}))
        this.setState({
            filterCat: value
        })
        hashHistory.push(`/socialMedia-twitter?page=1&type=${filter.type}&filterCat=${value}`)
        this.doSearch('init', {'currentPage': 1, filterCat: value})
    }
    ...
    unReadChange = (e) => {
        const {dispatch} = this.props
        dispatch(setFilterData({includeDupurl: e.target.checked}))
        this.doSearch('init', {'currentPage': 1, includeDupurl: e.target.checked})
    }
...
*/
    ...
    render () {
        ...
        let twitterTypeOptions = [
            ...
            {label: `策略`, value: '5', num: num.mars_strategy_clean || 0}
        ]
        return <div className="twitter-index">
            ...
            <Select defaultValue={`${location.query.filterCat || '0'}`} style={{ width: 80, marginRight: 10 }} onChange={this.handleImpChange}>
                <Option title='未导入' value='0'>未导入</Option>
                <Option title='算法导入' value='1'>算法导入</Option>
                <Option title='人工导入' value='2'>人工导入</Option>
                <Option title='全部' value='3'>全部</Option>
                <Option title='重要' value='4'>重要</Option>
                <Option title='不重要' value='5'>不重要</Option>
            </Select>
            <Checkbox defaultChecked={filter.includeRead} onChange={this.readChange}>包含已读</Checkbox>
            <Checkbox onChange={this.unReadChange}>包含重复</Checkbox>
            {/* <Checkbox defaultChecked={filter.includeUnread} onChange={this.unReadChange}>包含重复</Checkbox> */}
            <span>来源:</span>
            <Input
            ...
            <div className="mt10">
                <Spin spinning={this.state.loading} size="large">
                    <Table rowSelection={rowSelection} dataSource={list.map((item, index) => ({...item, key: index}))} columns={columns} bordered pagination={{current: parseInt(location.query.page) || parseInt(pageData.currentPage), total: pageData.totalCount, pageSize: pageData.pageSize, onChange: (page) => this.changePage(page)}} />
