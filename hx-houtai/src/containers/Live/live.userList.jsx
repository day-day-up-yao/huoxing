...
const userTypeHuoxinghao = [
    { label: '有MarsBit专栏', value: '1' },
    { label: '无MarsBit专栏', value: '0' },
    { label: '全部', value: '' }
]
class LiveUser extends Component {
    ...
    // 筛选是否关联MarsBit专栏
    handleChangeHuoxinghao = (value) => {
        this.setState({
            marsFlag: value
        })
        this.doSearch('init', {'page': 1, marsFlag: value})
    }
        ...
        return <div className="live-index">
            <Row>
                <Col>
                    ...
                    <span style={{ marginLeft: '15px' }}>MarsBit专栏：</span>
                    <Select defaultValue={`${filter.type}`} style={{width: 120}} onChange={this.handleChangeHuoxinghao}>
                        {userTypeHuoxinghao.map(d => <Option value={d.value} key={d.value}>{d.label}</Option>)}
                    </Select>
