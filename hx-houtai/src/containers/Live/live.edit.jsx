...
import {Form, Input, Upload, Icon, Modal, Button, message, Spin, DatePicker, Select, Radio} from 'antd'
    ...
    componentWillMount () {
        ...
        if (location.query.id) {
            dispatch(getLiveItemInfo({'castId': location.query.id}, (data) => {
                this.setState({
                    updateOrNot: true,
                    fileList: !data.backImage ? [] : [{
                        uid: 0,
                        name: 'xxx.png',
                        status: 'done',
                        url: data.backImage
                    }],
                    mFileList: !data.mBackImage ? [] : [{
                        uid: 0,
                        name: 'xxx.png',
                        status: 'done',
                        url: data.mBackImage
    ...
    handleSubmit = (e) => {
        ...
        this.props.form.validateFieldsAndScroll((err, values) => {
            if (!err) {
                ...
                values.beginTime = this.state.radioValue === '1' ? Date.parse(values['beginTime'].format('YYYY-MM-DD HH:mm')) : null
                values.guestId = values.guestId.join(',')
                values.presenterId = values.presenterId.join(',')
                ...
                <Form onSubmit={this.handleSubmit}>
                    ...
                    >
                        {getFieldDecorator('beginTime', {
                            rules: [{required: false, message: '请选择直播开始时间！'}],
                            initialValue: (updateOrNot && liveInfo) ? !liveInfo.beginTime ? null : this.state.radioValue === '2' ? null : moment(formatDate(liveInfo.beginTime), 'YYYY-MM-DD HH:mm:ss') : this.state.radioValue === '1' ? moment() : null
                        })(
                            <DatePicker showTime format="YYYY-MM-DD HH:mm:ss" style={{marginRight: 15}}/>
                        )}
                        <Radio.Group onChange={this.onChangeRadio} value={this.state.radioValue}>
                            <Radio value="1">固定时间</Radio>
                            <Radio value="2">非固定时间</Radio>
                        </Radio.Group>
                    </FormItem>
                    ...
                    >
                        {getFieldDecorator('guestId', (updateOrNot && liveInfo) ? {
                            initialValue: liveInfo.guestId.split(','),
                    ...
                    >
                        {getFieldDecorator('presenterId', (updateOrNot && liveInfo) ? {
                            initialValue: liveInfo.presenterId.split(','),
                    ...
                    >
                        <div className="dropbox">
                            {getFieldDecorator('backImage', {
                                ...
                                rules: [{required: false, message: '请上传直播背景图！'}]
                            })(
                                <Upload
                            ...
                            <span className="cover-img-tip">用于m端直播页面的背景图展示, 长宽比例: <font style={{color: 'red'}}>720 * 192</font></span>
                        </div>
                    </FormItem>
                    ...
                    >
                        <div className="dropbox">
                            ...
                            <span className="cover-img-tip">用于pc端直播页面的背景图展示, 长宽比例: <font style={{color: 'red'}}>820 * 136</font></span>
                        </div>
                    </FormItem>
