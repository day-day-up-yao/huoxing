...
import { Table, Row, Col, Modal, Input, message, Spin, Select, Button } from 'antd'
import './index.scss'
...
class LiveIndex extends Component {
                ...
                return (
                    <div>
                        <p style={{marginBottom: 10}}>
                            <Link className="mr10 opt-btn" href={`https://www.huoxing24.com/live/${record.id}`} target="_blank" style={{background: '#344399'}}>进入直播间</Link>
                        </p>
                        <p style={{marginBottom: 10}}>
                            <Link className="mr10 opt-btn" to={{pathname: '/live-detail', query: {id: record.id}}} style={{background: '#108ee9'}}>发布内容</Link>
                        </p>
                    </div>
                )
            }
    ...
    // 点击搜索
    _search (e) {
        this.doSearch('init', {'currentPage': 1, title: this.state.titleVal})
    }
    changeTitle = (val) => {
        this.setState({
            titleVal: val
