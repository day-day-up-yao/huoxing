...
import {getVideoList, setSearchQuery, setPageData, setFilterData, newsToTop, editVideoList, selectedData} from '../../actions/video/video.action'
...
class VideoIndex extends Component {
    ...
    handleRecommend = (item) => {
        ...
        confirm({
            ...
            onOk () {
                ...
                axiosAjax('POST', '/video/changeRecCreaterType', {...sendData}, (res) => {
                    if (res.code === 1) {
    ...
    componentWillMount () {
        ...
        columns = [{
            ...
            width: 250,
            key: 'name',
            render: (text, record) => (record && <div className="video-info clearfix">
                <div>
            ...
            render: (item) => (<div className="btns">
                ...
                <p><a className={`mr10 toMiniApp-btn opt-btn`} href="javascript:void(0)" style={{background: '#1048ae'}} onClick={() => this.handleRecommend(item)}>
                    {
            ...
            render: (item) => (<div className="btns">
                <p>
                    <Link target="_blank" className="mr10 opt-btn" to={{pathname: '/video-detail', query: {id: item.id}}} style={{background: '#108ee9'}}>详情</Link>
                </p>
                <p>
                    <Link target="_blank" className="mr10 opt-btn" to={{pathname: '/video-send', query: {id: item.id}}} style={{background: '#e07091'}}>编辑</Link>
                </p>
                <p>
                ...
                <p>
                    <a onClick={() => this.delVideo(item)} className="mr10 opt-btn" href="javascript:void(0)" style={{background: '#d73435'}}>删除</a>
                </p>
            </div>)
        }]
    }
    ...
    changeTopValue (e, record) {
        ...
        dispatch(editVideoList({topOrder: val}, record.key))
    }
    ...
    doSearch (type, data) {
        ...
        dispatch(getVideoList(type, sendData, () => {
            this.setState({
                loading: false
            })
        }))
                ...
                <Col>
                    ...
                    */}
                    ...
                    <span>
                        ...
                        <Button type="primary" style={{margin: '0 15px'}} onClick={() => { window.open(`${window.location.origin}/#/video-send`) }}>新增</Button>
                        <Button type="primary" onClick={() => { this.doSearch('init') }} shape="circle" icon="reload" />
                    </span>
                </Col>
            </Row>
            <div className="mt30">
...
const mapStateToProps = (state) => {
    return {
        ...
        list: state.videoInfo.list,
        search: state.videoInfo.search,
        filter: state.videoInfo.filter,
        pageData: state.videoInfo.pageData,
        selectedData: state.videoInfo.selectedData
    }
}

export default connect(mapStateToProps)(Form.create()(VideoIndex))
