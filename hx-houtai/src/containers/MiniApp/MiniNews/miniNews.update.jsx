...
import {getMiniAppItemInfo, getLocalInfo} from '../../../actions/miniApp/miniApp.action'
...
class MiniAppSend extends Component {
    constructor (props) {
        super(props)
        this.state = {
            editor: '',
    ...
    componentDidMount () {
        ...
        if (location.query.id) {
            dispatch(getMiniAppItemInfo({'id': location.query.id}, (res) => { this.renderData(res) }))
        } else {
        ...
        this.setState({
            ...
            this.props.form.validateFieldsAndScroll((err, values) => {
                if (!err) {
                    ...
                    axiosAjax('post', `${(this.state.updateOrNot && this.props.location.query.id) ? '/miniapp/update' : '/news/add'}`, values, (res) => {
                        this.setState({
                            loading: false
                        })
                        if (res.code && res.code === 1) {
                            ...
                            hashHistory.push('/miniNews-list')
                        } else {
                            this.setState({
                                iconLoading: false
                            })
    ...
    render () {
        ...
        const {cateId, focusImg, allfileList, uploadAllImgModal, progressNum, videoPcfileList, videoMfileList, videofileList, uploading, previewVisible, previewImage, fileList, pcfileList, mfileList, mcfileList, tags, inputVisible, inputValue, newsContent, updateOrNot, newsVisible, mp3fileList} = this.state
        const formItemLayout = {
            labelCol: {span: 1},
        ...
        return <div className="miniApp-send">
            {/*
                ...
                <Form onSubmit={this.handleSubmit}>
                    ...
                    <FormItem {...formItemLayout} label="类别: ">
                        {getFieldDecorator('cateId', {
                            ...
                            <RadioGroup
                                disabled={this.props.location.query.url}
                    ...
                    >
                        {getFieldDecorator('source', {
                            initialValue: (updateOrNot && newsInfo) ? `${newsInfo.source || 'MarsBit'}` : 'MarsBit',
                            rules: [{required: true, message: '请输入新闻来源！'}]
                        })(
                            <Input disabled={parseInt(cateId) === 1} className="news-source" placeholder="请输入新闻来源"/>
                        )}
                    </FormItem>
                    ...
                    {(videofileList[0] && videofileList[0].fileUrl) ? <div className="miniApp-cover">
                        {this.FormItem({
                            imgName: 'video_pc',
                            label: '视频PC封面',
                            imgUrl: '',
                    ...
                    >
                        ...
                        <div>
                            ...
                            <span>每个标签最多<font style={{color: 'red'}}> 8 </font>个字</span>
                        </div>
                    </FormItem>
...
const mapStateToProps = (state) => {
    return {
        userInfo: state.miniAppInfo.userInfo,
        newsInfo: state.miniAppInfo.info,
        channelList: state.channelListInfo
    }
}

export default connect(mapStateToProps)(Form.create()(MiniAppSend))
