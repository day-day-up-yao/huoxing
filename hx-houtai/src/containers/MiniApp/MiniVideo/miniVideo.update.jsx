...
class MiniVideoSend extends Component {
    constructor (props) {
        super(props)
        this.state = {
            updateOrNot: false,
    ...
    componentDidMount () {
        ...
        if (location.query.id || location.query.url) {
            dispatch(getMiniVideoItemInfo({'id': location.query.id}, (res) => { this.renderData(res) }))
        } else {
            this.setState({
                loading: false
            })
    ...
    // pc 视频展示图上传
    handleMiniVideoPcChange = ({file, fileList}) => {
        this.setState({
    ...
    // 手机端视频展示图上传
    handleMiniVideoMChange = ({file, fileList}) => {
        this.setState({
    ...
    handleUpload = () => {
        ...
        this.UploadMiniVideo(file, formData, totalSize, blockCount, blockSize)
    }

    UploadMiniVideo = (file, formData, totalSize, blockCount, blockSize) => {
        if (pause) {
            return // 暂停
        }
        try {
    ...
    handleSubmit = (e) => {
        ...
        this.props.form.validateFieldsAndScroll((err, values) => {
            if (!err) {
                ...
                axiosAjax('post', `${(this.state.updateOrNot && this.props.location.query.id) ? '/miniappvideo/update' : '/miniappvideo/add'}`, values, (res) => {
                    this.setState({
                        loading: false
                    })
                    if (res.code === 1) {
                        message.success(this.state.updateOrNot ? '修改成功！' : '添加成功！')
                        hashHistory.push('/miniVideo-list')
                    } else {
                        message.error(res.msg)
                    }
                })
    ...
    render () {
        ...
        return <div className="miniVideo-send">
                        ...
                        <div className="dropbox">
                            {getFieldDecorator('video_pc', {
                                ...
                                <Upload
                                    ...
                                    onPreview={this.handlePreview}
                                    onChange={this.handleMiniVideoPcChange}
                                >
                        ...
                        <div className="dropbox">
                            {getFieldDecorator('video_m', {
                                ...
                                <Upload
                                    ...
                                    onPreview={this.handlePreview}
                                    onChange={this.handleMiniVideoMChange}
                                >
...
const mapStateToProps = (state) => {
    return {
        userInfo: state.miniVideoInfo.userInfo,
        newsInfo: state.miniVideoInfo.info
    }
}

export default connect(mapStateToProps)(Form.create()(MiniVideoSend))
