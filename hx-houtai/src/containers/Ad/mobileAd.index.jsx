...
import {formatDate, axiosAjax, cutString, appAdPosition, sortDownByKey} from '../../public/index'
    ...
    componentWillMount () {
        ...
        columns = [{
            ...
            render: (record) => <div
                ...
                key={record.mImgSrc}
                style={{
                    background: `url(${record.mImgSrc}) no-repeat center / cover`
                }}
                src={record.mImgSrc}
            ...
            title: '权重',
            key: 'topOrder',
            render: (record) => (<p>{parseInt(this.adPosition(record)) !== 15 ? record.topOrder : record.weight}</p>)
        }, {
            title: '更新时间',
            key: 'updateTime',
            ...
            <Row>
                <Col span={22} className="ad-position">
                    ...
                    <Select defaultValue={`${filter.adMobilePlace}`} style={{ width: 140 }} onChange={this.handleChange}>
                        ...
                        {appAdPosition.map(d => <Option value={d.value} key={d.value}>{d.label}</Option>)}
                    </Select>
                    <Select defaultValue={`${filter.mStatus}`} style={{ width: 140, margin: '0 0 0 15px' }} onChange={this.handleStatusChange}>
                        <Option value="">全部</Option>
