...
import {formatDate, axiosAjax, cutString, pcAdPosition, sortDownByKey} from '../../public/index'
...
class AdIndex extends Component {
    ...
    componentWillMount () {
        ...
        columns = [{
            ...
            render: (record) => <div
                ...
                key={record.id}
                style={{
                    background: `url(${record.mImgSrc ? record.mImgSrc : record.pcImgSrc}) no-repeat center / cover`
                }}
                src={record.mImgSrc ? record.mImgSrc : record.pcImgSrc}
            ...
            title: '权重',
            key: 'topOrder',
            render: (record) => (<p>{parseInt(this.adPosition(record)) !== 3 && parseInt(this.adPosition(record)) !== 12 ? record.topOrder : record.weight}</p>)
        }, {
            title: '更新时间',
            key: 'updateTime',
    ...
    changePage (page) {
        ...
        this.doSearch(search.type, {'currentPage': page, adPlace: filter.adPcPlace, status: filter.pcStatus})
    }
    // 删除
    delAd (item) {
        const {dispatch} = this.props
        ...
        return <div className="ad-index">
            <Row>
                <Col span={22} className="ad-position">
                    ...
                    <Select defaultValue={`${filter.pcStatus}`} style={{ width: 140, margin: '0 0 0 15px' }} onChange={this.handleStatusChange}>
                        <Option value="">全部</Option>
            ...
            <div className="mt30">
                <Spin spinning={this.state.loading} size="large">
                    <Table dataSource={sortDownByKey(list, 'topOrder').map((item, index) => ({...item, key: index}))} columns={columns} bordered pagination={{current: pageData.currPage, total: pageData.totalCount, pageSize: pageData.pageSize, onChange: (page) => this.changePage(page)}} />
                </Spin>
            </div>
