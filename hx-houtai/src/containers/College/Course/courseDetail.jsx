...
class CourseDetail extends Component {
    ...
    componentDidMount () {
        ...
        dispatch(getCourseItemInfo({'id': location.query.id}, () => {
            this.setState({
                loading: false
            })
        }))
    ...
    _forbidcomment (forbidcomment) {
        ...
        axiosAjax('POST', '/course/forbidcomment', sendData, (res) => {
            if (res.status === 200) {
    ...
    _isTop (istop) {
        ...
        axiosAjax('POST', '/course/top', sendData, (res) => {
            if (res.status === 200) {
                // this.doSearch(this.state.type)
                dispatch(getCourseItemInfo({'id': location.query.id}))
            } else {
    ...
    render () {
        ...
        return <Spin spinning={this.state.loading} size="large">
            {info.id ? <div className="course-detail">
                <Row>
                    <Col span={1}>
                    ...
                    <Col className="text-right" span={20} offset={3}>
                        ...
                        <Button onClick={() => this._delCourse()}><IconItem type="icon-clear"/>删除</Button>
                        */}
                    </Col>
                </Row>
                ...
                */}
                {isJsonString(info.coverPic) ? <div style={{margin: '15px 0'}}>
                    ...
                    <Row className="news-cover-img">
                        <Col className="section">
                            <span className="name">PC-课程封面：</span>
                            <img
                    ...
                    <Row className="news-cover-img">
                        <Col className="section">
                            <span className="name" style={{width: '125px', verticalAlign: 'top'}}>M-课程封面：</span>
                            <img
                ...
                <Row className="news-title">
                    <Col className="section">
                        <span className="name">课程标题：</span>
                        <span className="desc">{`${info.title}`} </span>
                    </Col>
                </Row>
                <Row className="news-summary">
                    <Col className="section">
                        <span className="name">课程特色(简介)：</span>
                        <span className="desc">{`${info.synopsis}`} </span>
                    </Col>
                </Row>
                <Row>
                    <Col style={{fontSize: '15px', fontWeight: 'bolder', padding: '5px', color: '#000'}}>课程内容: </Col>
                    <Col className="page-box course-content simditor">
                        <div className="course-main simditor-body">
