...
class ChapterDetail extends Component {
    ...
    componentWillMount () {
        ...
        dispatch(getChapterItemInfo({'id': location.query.id}, () => {
            this.setState({
                loading: false
            })
        }))
    ...
    render () {
        ...
        return <div>
            <Spin spinning={this.state.loading} size="large">
                {info.id ? <div className="chapter-detail">
                    <Row>
                        {/*
                        <Col span={1}>
                        ...
                        */}
                        <Col className="">
                             ...
                             */}
                            <Button onClick={() => this._delChapter()}><IconItem type="icon-clear"/>删除</Button>
                        </Col>
                    </Row>
                                        ...
                                        if (JSON.parse(info.url).length !== 0) {
                                            return JSON.parse(info.url).map(function (item, index) {
                                                return <li className="clearfix" key={index} style={{margin: '5px 0'}}>
                                                    <chapter width="200" style={{verticalAlign: 'middle'}} src={item.fileUrl} preload="metadata" controls="controls"></chapter>
                                                </li>
                                            })
                                        } else {
                                            return <span>暂无</span>
                                ...
                                if (JSON.parse(info.url).length !== 0) {
                                    return <Row className="news-cover-img">
                                        <Col className="section">
                                            ...
                                            <img
                                                className="desc" onClick={() => this.showModal(JSON.parse(info.coverPic).chapter_pc)}
                                                src={`${JSON.parse(info.coverPic).chapter_pc}`}/>
                                        </Col>
                                        <Col className="section">
                                            <span className="name">M-播放器封面：</span>
                                            <img
                                                className="desc" onClick={() => this.showModal(JSON.parse(info.coverPic).chapter_m)}
                                                src={`${JSON.parse(info.coverPic).chapter_m}`}/>
                                        </Col>
                                    </Row>
                                } else {
                                    return ''
