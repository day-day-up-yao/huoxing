...
class ArticleAuditIndex extends Component {
    ...
    componentWillMount () {
        ...
        // },
        {
            ...
            key: 'action',
            width: 180,
            render: (item, record) => (<div className="huoxinghao-operation">
                {item.status === 4 ? <Link
                    target="_blank"
                    className="mr10 opt-btn"
    ...
    doBloacktDo = () => {
        ...
        if (this.state.reason.trim() !== '') {
            axiosAjax('POST', '/blacklist/addblacklist_by_nickname', {
                nickname: this.state.author, // 获取点击的id
    ...
    _isPublish (item) {
        ...
        confirm({
            title: '提示',
            content: `确认要${item.status === 0 ? '发表' : '草稿状态不可点击'}吗 ?`,
            onOk () {
                let sendData = {
                    // 'appId': $.cookie('gameId'),
                    id: item.id,
                ...
                axiosAjax('POST', '/news/status', {...sendData}, (res) => {
                    if (res.code === 1) {
                        message.success(`${item.status === 0 ? '发表' : '草稿可不操作'}成功`)
                        _this.doSearch('init')
                        dispatch(setSearchQuery({'type': 'init'}))
                        if (item.status === 4) {
    ...
    render () {
        ...
        return <div className="article-index">
            <Row>
                ...
                <Col>
                    <span>文章状态：</span>
                    <Select defaultValue="0" style={{ width: 120, marginBottom: 10 }} onChange={this.handleChange}>
