...
import { Table, Row, Col, Modal, Spin, Select, Input, Button, message } from 'antd'
import '../index.scss'
// import { hashHistory } from 'react-router'
import {getAuditList, setSearchQuery, setPageData, setFilterData, selectedData} from '../../../actions/audit/audit.action'
import {axiosAjax, formatDate, cutString, auditStatus} from '../../../public/index'
...
class AuditIndex extends Component {
            ...
            render: (item) => (<div>
                ...
                {(() => {
                    if (item.state === 0) {
                        ...
                        return <a
                            disabled
                            className="mr10 opt-btn disabled"
                        >
                            未提交信息
                        </a>
                    } else {
    ...
    detailModal (obj) {
        ...
        axiosAjax('get', '/account/getRealAuthByPassportId', params, (res) => {
            if (res.code === 1) {
                dispatch(selectedData(res.obj))
                window.sessionStorage.setItem('hx_identify_info', JSON.stringify(res.obj))
                window.location.href = `${window.location.origin}/#/audit-details?id=${obj.passportid}`
            } else {
