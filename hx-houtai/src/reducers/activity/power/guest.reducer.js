...
const powerGuestInfo = (
    ...
    switch (action.type) {
        case POWERGUEST.ADD_DATA:
            return {...state, ...action.data}
        case POWERGUEST.ADD_QUERY:
            return {...state, query: {..._query, ...action.data}}
        case POWERGUEST.SET_SEARCH_QUERY:
            return {...state, search: {...search, ...action.data}}
        case POWERGUEST.SET_PAGE_DATA:
            return {...state, pageData: {...pageData, ...action.data}}
        case POWERGUEST.SET_FILTER_DATA:
            return {...state, filter: {...filter, ...action.data}}
        case POWERGUEST.EDIT_LIST_ITEM:
